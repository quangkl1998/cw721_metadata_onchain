...
async function main(contract_name) {
  ...
  testerClient = await SigningCosmWasmClient.connectWithSigner(
    chainConfig.rpcEndpoint,
    testerWallet,
    { gasPrice }
  );
  ...
  const instantiateMsg = {
    ...
    symbol: "Buzz",
    minter: deployerAccount.address,
  };

  // instantiate contract
  console.log("2. Instantiating contract...");
  let instantiateResponse = await instantiate(
    storeCodeResponse.codeId,
    instantiateMsg
  );
